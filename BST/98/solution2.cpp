...
public:
    bool isValidBST(TreeNode *root) {
        return inorderHelper(root, nullptr, nullptr);
    }

private:
    // The result of inorder traversal of a BST is a sorted array.
    bool inorderHelper(TreeNode *root, int *maxVal, int *minVal) {
        if (!root) { return true; }
        if (maxVal && root->val >= *maxVal) { return false; }
        if (minVal && root->val <= *minVal) { return false; }
        return inorderHelper(root->left, &root->val, minVal) &&
               inorderHelper(root->right, maxVal, &root->val);
    }
};
